...
# <span style="font-family:Times New Roman; font-size:1.5em;">**Extent of Reaction**</span>
# 
# We will define a quantity called the ***extent of reaction***.  Here we'll use a pretty lose definition and just say that the extent of reaction is a measure of how "much" reaction has occurred. For convenience (and compatibility with stoichiometric coefficients), we will define the extent of reaction in molar units.   
# 
# An important thing to note about the extent of reaction is that it is something that is specific to a *reaction, $i$*, it is not something that is specific to a *species, $j$*. Therefore, we will define the extent of reaction for a specific reaction, $i$. We use the symbol $\xi_i$ to represent the extent of the "$i^{th}$ reaction.  
# 
# $$\xi_i = \textrm{moles of reaction $i$ that have occured}$$
# 
...
# 
# $$\xi_1 = \textrm{moles of reaction $1$ that have occured}$$
# 
...
# \end{align}
# 
# Here, we are only considering ammonia synthesis, so we will assign this reaction the number 1, and define an extent, $\xi_1$ that describes the moles of "ammonia synthesis reaction" that have occured.
# 
# $$\xi_1 = \textrm{moles of ammonia synthesis that have occured}$$
# 
...
# 
# \begin{align}
#     N_{N_2}    &= N_{N_{2,0}}    - 1 \, \xi_1 \\
#     N_{H_2}    &= N_{H_{2,0}}    - 3 \, \xi_1 \\
#     N_{N\!H_3} &= N_{N\!H_{3,0}} + 2 \, \xi_1 \\
# \end{align}
# 
...
# <span style="font-family:Times New Roman; font-size:1.5em;">**Dealing with multiple reactions**</span>
# 
# Now that we have a general understanding of how to use an extent of reaction, it is very easy to extend the concept to 2, 3, 5, 10, or 1000 reactions.  We simply define a single extent of reaction for each reaction in our network, and then we consider how that reaction impacts the number of moles of each species using stoichiometric coefficients, just as we did above. As an example, let's consider adding a couple of completely arbitrary reaction that occurs in parallel to ammonia synthesis.  Now, we have three reactions involving six different species:
# 
# \begin{align}
#     N_2 (g) + 3H_2 (g) &\leftrightharpoons 2N\!H_3 (g) \ \ \ \ \ \ \ \ \ \ (i = 1) \\
#     N_2 (g) + 2O_2 (g) &\leftrightharpoons 2N\!O_3 (g) \ \ \ \ \ \ \ \ \ \ (i = 2) \\
...
# \begin{align}
#     \xi_1 &= \textrm{moles of ammonia synthesis that have occured} \\
#     \xi_2 &= \textrm{moles of nitrogen oxidation that have occured} \\
#     \xi_3 &= \textrm{moles of hydrogen oxidation that have occured} \\
# \end{align}
# 
...
# 
# \begin{align}
#     N_{N_2}    &= N_{N_{2,0}}    - 1 \, \xi_1 - 1 \, \xi_2 + 0 \, \xi_3\\
#     N_{H_2}    &= N_{H_{2,0}}    - 3 \, \xi_1 + 0 \, \xi_2 - 2 \, \xi_3\\
#     N_{O_2}    &= N_{O_{2,0}}    - 0 \, \xi_1 - 2 \, \xi_2 - 1 \, \xi_3\\
#     N_{N\!H_3} &= N_{N\!H_{3,0}} + 2 \, \xi_1 + 0 \, \xi_2 + 0 \, \xi_3\\
#     N_{NO_2}   &= N_{NO_{2,0}}   + 0 \, \xi_1 + 2 \, \xi_2 + 0 \, \xi_3\\
#     N_{H_2O}   &= N_{H_2O_{0}}   + 0 \, \xi_1 + 0 \, \xi_2 + 2 \, \xi_3\\
# \end{align}
# 
# If we know the values of extents, for example:
# 
# \begin{align}
#     \xi_1 = 0.25 \ \textrm{moles} \\
#     \xi_2 = 0.05 \ \textrm{moles} \\
#     \xi_3 = 0.15 \ \textrm{moles} \\
# \end{align}
# 
...
#     \mathbf{N} &=
#         \begin{bmatrix}
#             N_{N_2} \\ N_{H_2} \\ N_{O_2} \\ N_{N\!H_3} \\ N_{NO_3} \\ N_{H_2O} \\
#         \end{bmatrix}
#     \\
#     \\
#     \boldsymbol{\nu} &=
...
# 
# This can be cumbersome for small reaction networks involving only a few species, but it is often a good idea to organizer your information this way for large reaction networks involving many species.  As I demonstrate below, it makes it pretty easy to reduce the number of lines of code you write.
...
# <span style="font-family:Times New Roman; font-size:1.5em;">**Fractional Conversion**</span>
# 
...
# $$X_A = -\frac{\nu_A \xi_1}{N_{A,0}}$$
# 
# Or we can define extent as a function of conversion:
# 
# $$\xi_1 = -\frac{N_{A,0}X_A }{\nu_A}$$
# 
# Either may be useful to us.  Now, we already saw that we were able to use reaction extents to define the molar quantity of each species in terms of a single variable that measures reaction progress (the extent).  **For a single reaction** we can do the same thing with fractional conversion.  This is a frequent approach in reactor design.  It is attractive for a few reasons, chief among them is that conversion, because it is a fractional quantity, always varies between 0 and 1.  Further, it is always positive. Finally, it is an ***intensive*** quantity that is independent of system size.  For that rason, it is a more practical metric than reaction extent in terms of physical significance. 
# 
# For a tangible example, consider whether 150 moles of extent is a significant amount of reaction progress or not.  It is hard to say--it depends on the size of our system.  If we started with 170 moles of A, then yes.  If we started with several million moles of A, then no.  In contrast, stating that our system achieves, e.g., 80\% conversion is unambiguous.
# 
...
# <span style="font-family:Times New Roman; font-size:1.5em;">**The General Case**</span>
# 
...
# <span style="font-family:Times New Roman; font-size:1.5em;">**Fractional Conversion in the Ammonia Synthesis Example**</span>
# 
...
# 
# $$\xi_1 = -\frac{N_{N_{2,0}}X_{N_2}}{\nu_{N_2}} = N_{N_{2,0}}X_{N_2} $$
# 
# In other words, $N_{N_{2,0}}X_A$ represents the moles of reaction that have occured.  We know that for every reaction that occurs, it consumes 1 mole of $N_2$, it consumes 3 moles of $H_2$, and it produces 2 moles of $N\!H_3$.  So:
# 
# \begin{align}
...
# \end{align}
# 
# **Notice**: We do not define a new fractional conversion for each species.  This defeats the purpose of the exercise.  We want to use a single fractional conversion for a single reactant to quantify the changes in each species.

# <span style="font-family:Times New Roman; font-size:1.5em;">**Be careful with stoichiometry!**</span>
# 
# You'll find that it is slightly trickier to keep track of changes in molar quantities with fractional conversion than with extent.  With extent, you use stoichiometric coefficients directly because the extent is specific to a reaction.  With conversions, you have to use ratios of stoichiometric coefficients because conversion is specific to a species.
# 
...
# <span style="font-family:Times New Roman; font-size:1.5em;">**A note about units**</span>
# 
# 
# **Batch Reactor**
# 
